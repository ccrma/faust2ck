...
char *ctrl_cget_funcs[] = {"CK_DLL_MFUN(%dsp_name%_ctrl_%var_name%)\n\
{\n\
...
void strip(char *to, const char *from, int quotes, int replace_spaces) {
  ...
  while (j > 0 && (to[j] == ' ' || to[j] == '\t' || (to[j] == '"' && quotes) ||
...
int do_example(FILE *exOut, char *machine_name) {
  char firstLetter = dspname[0];

  fprintf(exOut, "@import \"../%s.chug\"\n\n", machine_name);
...
void usage() { fprintf(stderr, "usage: faust2ck [-x] <filename.dsp>\n"); }
...
#define BUF_SIZE 1024
  ...
  if (result != 0) {
    fprintf(stderr,
            "error: unable to copy .dsp file to temporary work directory\n");
    rc = 5;
    goto error;
  }
  ...
  if (!write_header(".faust2ck_tmp/chugin.h", chuck_dl_h)) {
    fprintf(stderr, "error: unable to write ChucK header file to temporary "
                    "work directory\n");
    rc = 5;
    goto error;
  }
  ...
  snprintf(cmd, BUF_SIZE, "faust -xml '.faust2ck_tmp/%s' > /dev/null",
           dspfilename);
  // printf("%s\n", cmd);
  result = system(cmd);
  if (result != 0) {
  ...
  snprintf(cmd, BUF_SIZE, "sed '/\\<meta/d' %s > %s.tmp", xmlfilepath,
           xmlfilepath);
  printf("%s\n", cmd);
  result = system(cmd);
  if (result != 0) {
  ...
  if (!sourceCode) {
    snprintf(cmd, BUF_SIZE,
             "faust -a '%s' -o '.faust2ck_tmp/%s.cpp' '.faust2ck_tmp/%s'",
  ...
  if (!sourceCode) {
    snprintf(cmd, BUF_SIZE,
             "cc -I.faust2ck_tmp -arch x86_64 -arch arm64 -shared -O3 -fPIC %s "
             "-lc++ -o '%s.chug' '.faust2ck_tmp/%s.cpp'",
  ...
  if (!sourceCode) {
    snprintf(cmd, BUF_SIZE,
             "cc -I.faust2ck_tmp -shared -fPIC -O3 -lstdc++ %s -o '%s.chug' "
             "'.faust2ck_tmp/%s.cpp'",
             debugOption, basename, dspfilename);
    // printf("%s\n", cmd);
    result = system(cmd);
...
#elif defined(_WIN32)
  /* VSCode project */
  snprintf(cmdChuginate, BUF_SIZE,
           "./chuginate/chuginate %s && cp %s.dsp.cpp %s/%s.cpp", basename,
...
#error no target platform (e.g. Mac OS, Linux)
  ...
  if (generateExample) {
    snprintf(exampleFilename, BUF_SIZE, "%s-test.ck", basename);
